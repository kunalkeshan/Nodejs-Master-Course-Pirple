        ...
        if (!err && checkIds && checkIds.length > 0) {
            ...
            checkIds.forEach(function (checkId) {
                _data.read('checks', checkId, function (err, checkData) {
                    ...
                    // If the user has specified any state or none, include the current check
...
cli.responders.moreCheckInfo = function (str) {
    ...
    if (checkId) {
        // Look the log data
        _data.read('checks', checkId, function (err, checkId) {
            if (!err && checkId) {
                // Print JSON with text highlighted
                cli.verticalSpace();
...
cli.responders.moreLogInfo = function (str) {
    ...
    if (logFileName) {
        ...
        _logs.decompress(logFileName, function (err, strData) {
            if (!err && strData) {
                // split into lines
                var lineArr = strData.split('\n');
                lineArr.forEach(function (jsonString) {
